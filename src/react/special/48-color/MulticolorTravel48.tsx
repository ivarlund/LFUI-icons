import * as React from "react";
interface SVGRProps {
  title?: string;
  titleId?: string;
}

function MulticolorTravel48({
  title,
  titleId,
  ...props
}: React.SVGProps<SVGSVGElement> & SVGRProps) {
  return (
    <svg width={48} height={48} aria-labelledby={titleId} {...props}>
      {title ? <title id={titleId}>{title}</title> : null}
      <g fillRule="nonzero" fill="none">
        <path
          d="M17.5 4c-3.112 0-5.707 2.188-6.175 5.066C8.319 9.494 6 11.978 6 15c0 3.325 2.808 6 6.25 6H15v-2h-2.75C9.892 19 8 17.198 8 15s1.892-4 4.25-4h1v-1c0-2.198 1.892-4 4.25-4s4.25 1.802 4.25 4v1h1c2.358 0 4.25 1.802 4.25 4 0 1.065-.444 2.064-1.228 2.811l1.38 1.448C28.325 18.141 29 16.622 29 15c0-3.02-2.32-5.505-5.325-5.933C23.207 6.188 20.612 4 17.501 4z"
          fill="#075BA0"
        />
        <path
          d="M23 20a2 2 0 10-4 0 2 2 0 004 0zm2 0a4 4 0 11-8 0 4 4 0 018 0z"
          fill="#E30613"
        />
        <path
          fill="#005AA0"
          d="M23.015 30.388l5.586 1.208.423-1.955-6.082-1.315-6.24 1.824 3.553 3.335 1.369-1.459-1-.939z"
        />
        <path
          d="M20.934 39.226l-9.636-1.096-5.002-4.698 6.516-1.835 3.352 1.805 2.515-.735 17.749-5.187 5.941 1.278.167.572a3.488 3.488 0 01-2.37 4.326l-7.76 2.269-5.346 5.696-6.26 1.829 1.144-4.503-1.01.279zm1.576-2.511l.203-.802 1.939.492-1.051 4.143 2.374-.694 5.346-5.696.261-.076 8.024-2.346a1.488 1.488 0 001.065-1.298l-4.168-.896-17.263 5.044-3.302.965-3.362-1.81-2.33.657 1.937 1.82 8.592.977 1.735-.48zm-.566 2.232l.566-2.232-.566 2.232zM27.989 24.03a1.001 1.001 0 00-.755.352.968.968 0 00-.234.644.993.993 0 001.012.974.992.992 0 00.988-.996.993.993 0 00-1.011-.974zm.046 3.97A2.993 2.993 0 0125 25.05a2.968 2.968 0 01.713-1.967 3 3 0 012.253-1.052A2.993 2.993 0 0131 24.98c.019 1.65-1.31 3-2.965 3.019z"
          fill="#005AA0"
        />
      </g>
    </svg>
  );
}

export default MulticolorTravel48;
